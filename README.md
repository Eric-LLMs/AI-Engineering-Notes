...
* **[DeepDive](https://github.com/Eric-LLMs/DeepDive):** Production-grade AI system for personalized learning — agentic tutoring, dual-track memory, hybrid RAG, personal/team cloud storage, and self-hosted LLM infrastructure. Highlights:
  - **Config-driven RAG pipeline**: plug-and-play retrieval nodes (vector + keyword recall, RRF fusion, cross-encoder rerank, parent-expand, relevance check) that you add / remove / reorder / toggle live from the admin console — no code, no restart; chunking preview, golden-set Eval (Recall@k / Precision@k / MRR), degrade-never-break retrieval, and vision-LLM PDF table transcription.
  - **Agent kernel**: ReactLoopAgent step loop with dual-track memory (PG tsvector + pgvector via RRF, recency-weighted), a skill catalog, hot-reloadable, dependency-injected plugins, and a read-only sandbox gating every tool call (READ / WRITE / NETWORK).
  - **Layered, cache-friendly prompts & tools**: byte-stable head (identity + one-line tool catalog) + per-step dynamic tail (memory / skills / injected) — warm prefix cache, cache identity measurable; deferred tools — a searchable one-line catalog, matched tools mount as stable stubs, full schemas via the search result.
